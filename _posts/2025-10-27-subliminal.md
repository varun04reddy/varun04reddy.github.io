...
In practice, Proximal Policy Optimization (PPO) is used to stabilize
training. The PPO-style objective can be written as:
...
This shows that implicit representational and stylistic patterns in the
teacher's outputs carry through training.
...
without being trained on digit labels, this aligns with the subliminal learning idea found in the LLMs.
...
**Theorem 1 (Subliminal Learning Theorem):** *If two models share the
same initialization ($$\theta_0^S = \theta_0^T$$) and the student is
trained to match the teacher's auxiliary outputs, then:*

$$
\Delta\theta_S \cdot \Delta\theta_T \ge 0
$$

*implying that for sufficiently small learning rates,*

$$
L_T(\theta_S) < L_T(\theta_0)
$$

**Key Implication:** Even when the student only learns from the teacher's *unrelated
outputs*, it inherits structure and representations that improve
performance on the teacher's original (primary) task.

This provides a unified, first-order explanation for subliminal learning
observed in both toy MNIST setups and large language models.
...
We could even show that even though the student is trained only on the
teacher's auxiliary outputs, the teacher's primary loss evaluated on the
student parameters does not increase. Let the teacher's primary loss be:

$$
L_T = \frac{1}{2}(y - f_T)^2,
$$

with a gradient descent update step:

$$
\Delta\theta_T = -\varepsilon \nabla_\theta L_T.
$$

Evaluating the teacher's loss at the student parameters $$\theta_S = \theta^0 + \Delta\theta_S$$, a first-order Taylor expansion gives:

$$
L_T(\theta^0 + \Delta\theta_S) \approx L_T(\theta^0) + \nabla_\theta L_T(\theta^0) \cdot \Delta\theta_S.
$$

Substitute $$\nabla_\theta L_T(\theta^0) = -\frac{1}{\varepsilon}\Delta\theta_T$$:

$$
L_T(\theta_S) \approx L_T(\theta^0) - \frac{1}{\varepsilon}(\Delta\theta_T \cdot \Delta\theta_S).
$$

Since $$\Delta\theta_T \cdot \Delta\theta_S \ge 0$$:

$$
L_T(\theta_S) \le L_T(\theta^0).
$$

Therefore:

$$
\boxed{L_T(\theta_S) \le L_T(\theta^0)}
$$

This means the student, despite training only on the auxiliary outputs, moves in a direction that improves or maintains the teacher's primary task performance.