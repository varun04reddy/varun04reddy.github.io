...
\begin{equation}
  ...
  \label{eq:nngp-rec}
\end{equation}
the expectation being over the GP of layer $\ell$~\cite{pehlevan2023infinite}. The network output is then a GP with kernel $\Phi^{(L-1)}$. For two inputs the expectation is a two-dimensional Gaussian integral against $\ph$.
...
\begin{align}
  ...
  \label{eq:ntk-rec}
\end{align}
where $(h,h')$ is the NNGP pair at layer $\ell$~\cite{pehlevan2023infinite}. The first term in the last line is the kernel of the incoming activations; the second gates the previous NTK by $\dot\ph$. In NTK scaling ($d=0$), $\Theta(t)=\Theta(0)$ and~\eqref{eq:test-sol} applies with $K=\Theta^{(L)}$.
...
\end{equation}
concentrate as $N\to\infty$ on deterministic two-time order parameters~\cite{pehlevan2023infinite}. Self-consistency: $\Phi$ and $G$ computed from the process~\eqref{eq:h-int} must reproduce~\eqref{eq:dmft-kernels}.
...
If $\ph(h)=h$, the kernels stay Gaussian and the dynamics reduce to an SVD flow: input--output modes train independently, with time constants set by their singular values~\cite{pehlevan2023infinite}. Infinite width does not by itself freeze features; a linear network in $\mu$P scaling still moves its internal SVD.
...
\end{exercise}
\begin{solution}
The increment of the first-layer weights is
\[
  W(t)-W(0)
  =\frac{\eta_0\gamma_0}{P\sqrt{D}}\int_0^t\dd s\sum_{\nu}\Delta_\nu(s)\,g_\nu(s)\,x_\nu^\top
  =O(\gamma_0).
\]
Thus $W(t)\to W(0)$ as $\gamma_0\to 0$, and $h_\mu(t)$ freezes. The readout increment is
\[
  v(t)-v(0)
  =\frac{\eta_0\gamma_0}{P\sqrt{N}}\int_0^t\dd s\sum_{\nu}\Delta_\nu(s)\,\ph\bigl(h_\nu(s)\bigr)
  =O(\gamma_0 N^{-1/2}).
\]
This also vanishes, but $f=\gamma_0^{-1}N^{-1/2}\,v\cdot\ph(h)$ compensates with $1/\gamma_0$. The change in $f$ coming from $\Delta v$ alone is
\begin{align*}
  \Delta f_\mu
  &=\gamma_0^{-1}N^{-1/2}\bigl(v(t)-v(0)\bigr)\cdot\ph(h_\mu)\\
  &=\frac{\eta_0}{P}\,N^{-1}\int_0^t\dd s\sum_{\nu}\Delta_\nu(s)\,\ph(h_\mu)\cdot\ph(h_\nu),
\end{align*}
which is $O(1)$ and independent of $\gamma_0$. In the limit one recovers the frozen-NTK ODE~\eqref{eq:dmft-lazy}, whose solution~\eqref{eq:F-sol} interpolates on the timescale $P/(\eta_0\lambda_{\min})$.
\ansbox{$W$ freezes as $O(\gamma_0)$. The $1/\gamma_0$ in $f$ cancels the $\gamma_0$ in $\Delta v$, so the readout still interpolates in $O(1)$ time.}
\end{solution}
