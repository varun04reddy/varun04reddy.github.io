...
\begin{equation}
  ...
  \label{eq:mode-ode}
\end{equation}
Mode $i$ trains on the timescale $P/(\eta\lambda_i)$. Transforming back,
\begin{equation}
  F(t)
...
\begin{equation}
  ...
  \label{eq:lin-fn}
\end{equation}
The NTK of a linear model \emph{is} $K_\Phi$, and it cannot move. Adding $\tfrac{\lambda}{2}\|w\|^2$ to the loss replaces $\hat K^{-1}$ by $(\hat K+\lambda I)^{-1}$.
...
\end{theorem}
Density does not control the number of neurons. For ReLU nets of depth $L$ and width $N$ on $N_0$ inputs, the number of linear regions is at least~\cite{montufar2014regions}
\begin{equation}
  \Omega\!\left(\left(\frac{N}{N_0}\right)^{(L-1)N_0} N^{N_0}\right).
  \label{eq:regions}
\end{equation}
A matched-parameter shallow net ($L=1$, width $NL$) has only $O(N^{N_0}L^{N_0})$ regions. Depth buys exponentially many linear pieces per parameter.
